...
class MainWindow(Gtk.ApplicationWindow):
    ...
    def on_configure_event(self, widget, event):
        ...
        self._internal_state['window-size'] = [size[0], size[1]]

        position = widget.get_position()
        self._internal_state['window-position'] = [position[0], position[1]]

    def on_window_state_event(self, widget, event):
        maximized = (
            'GDK_WINDOW_STATE_MAXIMIZED' in event.new_window_state.value_names)
        self._internal_state['window-maximized'] = maximized

    def save_state(self):
        state = {
            'docview': self.docview.save_state(),
            'window-size': self._internal_state.get('window-size', None),
            'window-position': self._internal_state.get(
                'window-position', None),
            'window-maximized': self._internal_state.get(
    ...
    def restore_state(self, state):
        ...
        if state.get('window-maximized'):
            self.maximize()
        size_setting = state.get('window-size')
        if size_setting is not None:
            self.resize(size_setting[0], size_setting[1])
        position_setting = state.get('window-position')
